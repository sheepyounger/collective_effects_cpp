...
longitimpe::LongitudinalImpedance::LongitudinalImpedance(
...
longitimpe::LongitudinalImpedance::LongitudinalImpedance(
    const longitwake::LongitudinalWakefield& wakefield
): scale_(wakefield.scale_)
{
...
longitimpe::LongitudinalImpedance::read_impedance_data(const std::string& filename)
{
    ...
    for (size_t i = 0; i < n; ++i){
        ...
        full_real[n - 1 + i] = pos_real[i] * scale_;
        full_imag[n - 1 + i] = pos_imag[i] * scale_;
    }
    // 负频率
...
tuple<vector<double>, vector<double>, vector<double>>
longitimpe::LongitudinalImpedance::wakefield_to_impedance(const longitwake::LongitudinalWakefield& wakefield)
{
    // 获取尾场数据的坐标范围
    ...
    float max_freq = 30e9;    //  最大频率 500GHz
    ...
    for (const auto& z : impe) {
        real_impe.push_back(z.real());
        imag_impe.push_back(z.imag());
    }
